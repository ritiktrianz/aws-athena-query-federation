...
public class BigQueryQueryBuilder
{
    ...
    public String build()
    {
        ...
        LOGGER.info("      StringTemplate Build Process:");
        LOGGER.info("      Schema: {}", schemaName);
        LOGGER.info("      Table: {}", tableName);
        LOGGER.info("      Projection: {}", projection);
        LOGGER.info("      Conjuncts: {}", conjuncts);
        LOGGER.info("      Order By: {}", orderByClause);
        LOGGER.info("      Limit: {}", limitClause);
        LOGGER.info("      Parameter Values: {}", parameterValues);

        query.add(TEMPLATE_FIELD, this);
        String result = query.render().trim();
        
        LOGGER.info("    StringTemplate Rendered SQL: {}", result);
        return result;
    }
