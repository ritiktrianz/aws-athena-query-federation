...
public class OracleRecordHandlerTest
{
    ...
    public void buildSqlWithNullableComparisonsTest() throws SQLException {
        ...
        Mockito.when(nullValueSet.isAll()).thenReturn(false);
        Mockito.when(nullValueSet.getRanges().getOrderedRanges()).thenReturn(Collections.emptyList());

        ValueSet notNullValueSet = Mockito.mock(SortedRangeSet.class, Mockito.RETURNS_DEEP_STUBS);
        Mockito.when(notNullValueSet.isNullAllowed()).thenReturn(false);
        Mockito.when(notNullValueSet.isAll()).thenReturn(false);
        Mockito.when(notNullValueSet.getRanges().getOrderedRanges()).thenReturn(Collections.emptyList());

        Map<String, ValueSet> summary = new ImmutableMap.Builder<String, ValueSet>()
                .put("nullableCol1", nullValueSet)
                .put("nullableCol2", notNullValueSet)
                .build();
        ...
        String expectedSql = "SELECT \"nullableCol1\", \"nullableCol2\" FROM \"testSchema\".\"testTable\" PARTITION (p0)  WHERE (\"nullableCol1\" IS NULL) AND (\"nullableCol2\" IS NOT NULL)";
        PreparedStatement expectedPreparedStatement = createMockPreparedStatement(expectedSql);
    ...
    public void testEmptyConstraintsWithOrderBy() throws SQLException {
        ...
        String expectedSql = "SELECT \"" + COL_ID + "\", \"" + COL_NAME + "\" FROM \"testSchema\".\"testTable\" PARTITION (p0) ORDER BY \"" + COL_ID + "\" ASC, \"" + COL_NAME + "\" DESC";
        PreparedStatement preparedStatement = createMockPreparedStatement(expectedSql);
