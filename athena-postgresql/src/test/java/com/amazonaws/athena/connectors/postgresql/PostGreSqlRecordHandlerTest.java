...
public class PostGreSqlRecordHandlerTest extends TestBase
{
    ...
    // Testing complex WHERE clause generation from constraints
            ...
            throws SQLException
    {
        ...
        // When: Complex constraints with multiple predicates are applied
        ...
        // Then: SQL should have complex WHERE clause with all predicates
            ...
            throws SQLException
    {
        ...
        // Test simple string and int constraints (simulating nullable vs required fields)
            ...
            throws SQLException
    {
        ...
        // Test different numeric data types with ranges
    ...
    // ========== LIMIT AND TOP N TESTS ==========
    // Testing LIMIT clause generation from constraints

    @Test
    public void buildSqlWithLimitTest()
            throws SQLException
    {
        // Given: Schema with basic fields and LIMIT constraint
        ...
        String expectedSql = String.format("SELECT \"%s\", \"%s\" FROM \"%s\".\"%s\"  LIMIT 100", 
                COL_ID, COL_NAME, PARTITION_SCHEMA, PARTITION_NAME);
            ...
            throws SQLException
    {
        ...
        String expectedSql = "SELECT \"id\", \"status\" FROM \"s0\".\"p0\"  WHERE ((\"id\" >= ? AND \"id\" < ?)) AND (\"status\" IN (?,?)) LIMIT 25";
        PreparedStatement expectedPreparedStatement = createMockPreparedStatement(expectedSql);
    ...
    // ========== ORDER BY TESTS ==========
    // Testing ORDER BY clause generation from constraints

    @Test
    public void buildSqlWithOrderByTest()
            throws SQLException
    {
        // Given: Schema with fields and ORDER BY constraint
        Schema schema = createSqlTestSchema(COL_ID, COL_NAME);
        TableName tableName = new TableName(TEST_SCHEMA, TEST_TABLE);
        ...
        // When: SQL is generated with ORDER BY constraint
        // Then: SQL should include ORDER BY clause with correct direction and nulls handling
            ...
            throws SQLException
    {
        ...
        // Create constraints with multiple ORDER BY columns
    ...
    // ========== COMBINATION TESTS ==========
    // Testing complex scenarios combining expressions, ORDER BY, and LIMIT

    @Test
    public void buildSqlWithAllFeaturesTest()
            throws SQLException
    {
        // Given: Complex schema with multiple data types
        Map<String, ArrowType> fieldTypes = new ImmutableMap.Builder<String, ArrowType>()
        ...
        // When: Complex constraints with WHERE, ORDER BY, and LIMIT are applied
        ...
        // Then: SQL should include complex WHERE, ORDER BY, and LIMIT clauses
        String expectedSql = String.format("SELECT \"%s\", \"%s\", \"%s\", \"%s\", \"%s\" FROM \"%s\".\"%s\"  WHERE ((\"%s\" >= ? AND \"%s\" < ?)) AND (\"%s\" IN (?,?,?)) AND ((\"%s\" > ? AND \"%s\" <= ?)) AND ((\"%s\" >= ? AND \"%s\" < ?)) AND (\"%s\" = ?) ORDER BY \"%s\" ASC NULLS FIRST, \"%s\" DESC NULLS LAST, \"%s\" ASC NULLS LAST LIMIT 20",
                ...
                COL_CATEGORY, COL_RATING, COL_PRICE);
            ...
            throws SQLException
    {
        ...
        String expectedSql = "SELECT \"employee_id\", \"department\", \"salary\", \"hire_date\" FROM \"s0\".\"p0\"  WHERE (\"department\" = ?) AND ((\"salary\" >= ? AND \"salary\" < ?)) ORDER BY \"salary\" DESC NULLS LAST, \"hire_date\" ASC NULLS FIRST LIMIT 5";
        PreparedStatement expectedPreparedStatement = createMockPreparedStatement(expectedSql);
