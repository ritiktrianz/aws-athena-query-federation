...
public class PostGreSqlSqlUtilsTest
{
    ...
    public void setUp() throws Exception
    {
        ...
        // Mock PreparedStatement and ResultSet for getCharColumns query
        java.sql.PreparedStatement preparedStatement = mock(java.sql.PreparedStatement.class);
    ...
    @Test
    public void buildSql_WithPartitionClause_GeneratesQueryWithPartitionTable()
    {
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = getConstraints(constraintMap, Collections.emptyList());
        ...
        String expectedSql = "SELECT null FROM \"partition_schema\".\"partition_table\"";

        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = PostGreSqlSqlUtils.buildSql(connection, tableName, schema, constraints, splitWithPartition, parameterValues);
    ...
    public void buildSql_WithAllPartitions_DoesNotIncludePartitionClause()
    {
        ...
        when(splitWithAllPartitions.getProperty(PostGreSqlMetadataHandler.BLOCK_PARTITION_SCHEMA_COLUMN_NAME)).thenReturn(PostGreSqlMetadataHandler.ALL_PARTITIONS);
        when(splitWithAllPartitions.getProperty(PostGreSqlMetadataHandler.BLOCK_PARTITION_COLUMN_NAME)).thenReturn(PostGreSqlMetadataHandler.ALL_PARTITIONS);

        String expectedSql = "SELECT null FROM \"test_schema\".\"test_table\"";

        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = PostGreSqlSqlUtils.buildSql(connection, tableName, schema, constraints, splitWithAllPartitions, parameterValues);
    ...
    public void buildSql_WithEmptySchema_GeneratesQueryWithNull()
    {
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = PostGreSqlSqlUtils.buildSql(connection, tableName, emptySchema, constraints, split, parameterValues);
    ...
    @Test
    public void buildSql_WithCharColumn_GeneratesQueryWithRTrim() throws Exception
    {
        ...
        when(charColumnsResultSet.getString("column_name")).thenReturn("charCol");
        
        ValueSet charSet = SortedRangeSet.newBuilder(STRING_TYPE, false)
                .add(new Range(Marker.exactly(allocator, STRING_TYPE, "test"), Marker.exactly(allocator, STRING_TYPE, "test")))
                .build();
        constraintMap.put("charCol", charSet);
        ...
        List<TypeAndValue> expectedParams = new ArrayList<>();
        expectedParams.add(new TypeAndValue(STRING_TYPE, "test"));
        String expectedSql = "SELECT RTRIM(\"charCol\") AS \"charCol\" FROM \"test_schema\".\"test_table\"  WHERE (\"charCol\" = ?)";
        
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = PostGreSqlSqlUtils.buildSql(charConnection, tableName, schema, constraints, split, parameterValues);
    ...
    @Test
    public void buildSql_WithPartitionAndConstraints_GeneratesCompleteQuery()
    {
        ValueSet rangeSet = SortedRangeSet.newBuilder(INT_TYPE, false)
                .add(new Range(Marker.above(allocator, INT_TYPE, 5), Marker.below(allocator, INT_TYPE, 15)))
                .build();
        constraintMap.put("intCol", rangeSet);
        ...
        // Create split with partition information
        Map<String, String> splitProperties = new LinkedHashMap<>();
        splitProperties.put(PostGreSqlMetadataHandler.BLOCK_PARTITION_SCHEMA_COLUMN_NAME, "part_schema");
        splitProperties.put(PostGreSqlMetadataHandler.BLOCK_PARTITION_COLUMN_NAME, "part_table");
        Split splitWithPartition = mock(Split.class);
        when(splitWithPartition.getProperties()).thenReturn(splitProperties);
        when(splitWithPartition.getProperty(PostGreSqlMetadataHandler.BLOCK_PARTITION_SCHEMA_COLUMN_NAME)).thenReturn("part_schema");
        when(splitWithPartition.getProperty(PostGreSqlMetadataHandler.BLOCK_PARTITION_COLUMN_NAME)).thenReturn("part_table");

        List<TypeAndValue> expectedParams = new ArrayList<>();
        expectedParams.add(new TypeAndValue(INT_TYPE, 5));
        expectedParams.add(new TypeAndValue(INT_TYPE, 15));
        String expectedSql = "SELECT \"intCol\" FROM \"part_schema\".\"part_table\"  WHERE ((\"intCol\" > ? AND \"intCol\" < ?))";
    ...
    @Test
    public void buildSql_WithComplexConstraints_GeneratesQueryWithMultiplePredicates()
    {
        ValueSet intRange = SortedRangeSet.newBuilder(INT_TYPE, false)
                .add(new Range(Marker.exactly(allocator, INT_TYPE, 10), Marker.exactly(allocator, INT_TYPE, 10)))
                .add(new Range(Marker.exactly(allocator, INT_TYPE, 20), Marker.exactly(allocator, INT_TYPE, 20)))
                .build();
        ValueSet stringRange = SortedRangeSet.newBuilder(STRING_TYPE, false)
                .add(new Range(Marker.above(allocator, STRING_TYPE, "a"), Marker.below(allocator, STRING_TYPE, "z")))
                .build();
        
        constraintMap.put("intCol", intRange);
        constraintMap.put("stringCol", stringRange);
        ...
        expectedParams.add(new TypeAndValue(INT_TYPE, 10));
        expectedParams.add(new TypeAndValue(INT_TYPE, 20));
        expectedParams.add(new TypeAndValue(STRING_TYPE, "a"));
        expectedParams.add(new TypeAndValue(STRING_TYPE, "z"));
        String expectedSql = "SELECT \"intCol\", \"stringCol\" FROM \"test_schema\".\"test_table\"  WHERE (\"intCol\" IN (?,?)) AND ((\"stringCol\" > ? AND \"stringCol\" < ?))";
    ...
    {
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = PostGreSqlSqlUtils.buildSql(connection, tableName, schema, constraints, split, parameterValues);
