...
public class PostGreSqlPredicateBuilderTest
{
    ...
    public void buildConjuncts_WithPartitionColumn_FiltersOutPartitionColumn()
    {
        ValueSet rangeSet = SortedRangeSet.newBuilder(INT_TYPE, false)
                ...
                .build();
        constraintMap.put("intCol", rangeSet);
        constraintMap.put(PostGreSqlMetadataHandler.BLOCK_PARTITION_COLUMN_NAME, rangeSet);

        fields.add(Field.nullable("intCol", INT_TYPE));
        fields.add(Field.nullable(PostGreSqlMetadataHandler.BLOCK_PARTITION_COLUMN_NAME, INT_TYPE));

        // Create split with partition column
        Map<String, String> splitProperties = new HashMap<>();
        splitProperties.put(PostGreSqlMetadataHandler.BLOCK_PARTITION_COLUMN_NAME, "p0");
        ...
        assertEquals("Should only have one conjunct", 1, conjuncts.size());
    }
    ...
    private List<String> buildConjuncts(Map<String, ValueSet> constraintMap, List<Field> fields, Split customSplit)
    {
        ...
        PostGreSqlPredicateBuilder predicateBuilder = new PostGreSqlPredicateBuilder();
