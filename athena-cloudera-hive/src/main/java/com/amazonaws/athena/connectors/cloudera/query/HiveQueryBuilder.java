...
import static com.amazonaws.athena.connectors.cloudera.HiveConstants.BLOCK_PARTITION_COLUMN_NAME;
import static com.amazonaws.athena.connectors.cloudera.HiveConstants.HIVE_QUOTE_CHARACTER;

public class HiveQueryBuilder extends JdbcQueryBuilder<HiveQueryBuilder>
{
    public HiveQueryBuilder(ST template)
    {
        super(template, HIVE_QUOTE_CHARACTER);
    }
    
    @Override
    protected JdbcPredicateBuilder createPredicateBuilder()
    {
        return new HivePredicateBuilder();
    }
    
    @Override
    protected List<String> getPartitionWhereClauses(Split split)
    {
        String partitionClause = split.getProperty(BLOCK_PARTITION_COLUMN_NAME);
