...
public class HiveSqlUtilsTest
{
    ...
    public void buildSql_WithBasicQuery_GeneratesSelectFromQuery()
    {
        ...
        String expectedSql = "SELECT null FROM test_catalog.test_schema.test_table";
    ...
    public void buildSql_WithConstraintsRanges_GeneratesQueryWithWhereClause()
    {
        ...
        String expectedSql = "SELECT intCol FROM test_catalog.test_schema.test_table  WHERE ((intCol > ? AND intCol < ?))";
    ...
    public void buildSql_WithInPredicate_GeneratesQueryWithInClause()
    {
        ...
        String expectedSql = "SELECT intCol FROM test_catalog.test_schema.test_table  WHERE (intCol IN (?,?,?))";
    ...
    public void buildSql_WithOrderBy_GeneratesQueryWithOrderBy()
    {
        ...
        List<TypeAndValue> expectedParams = new ArrayList<>();
        expectedParams.add(new TypeAndValue(INT_TYPE, 10));
        String expectedSql = "SELECT intCol FROM test_catalog.test_schema.test_table  WHERE (intCol = ?) ORDER BY intCol ASC NULLS FIRST, stringCol DESC NULLS LAST";
    ...
    public void buildSql_WithNullPredicate_GeneratesQueryWithIsNull()
    {
        ...
        List<TypeAndValue> expectedParams = Collections.emptyList();
        String expectedSql = "SELECT intCol FROM test_catalog.test_schema.test_table  WHERE (intCol IS NULL)";
    ...
    public void buildSql_WithNotNullPredicate_GeneratesQueryWithIsNotNull()
    {
        ...
        List<TypeAndValue> expectedParams = Collections.emptyList();
        String expectedSql = "SELECT intCol FROM test_catalog.test_schema.test_table  WHERE (intCol IS NOT NULL)";
        
        executeAndVerify(constraints, schema, expectedParams, expectedSql);
    }

    @Test
    public void buildSql_WithPartitionClause_GeneratesQueryWithPartition()
    {
        Map<String, ValueSet> constraintMap = new LinkedHashMap<>();
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = getConstraints(constraintMap, Collections.emptyList());
        ...
        String expectedSql = "SELECT null FROM test_catalog.test_schema.test_table  WHERE p0 = 'value'";

        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = HiveSqlUtils.buildSql(CATALOG_NAME, tableName, schema, constraints, splitWithPartition, parameterValues);
    ...
    public void buildSql_WithMultipleColumns_GeneratesQueryWithAllColumns()
    {
        ...
        String expectedSql = "SELECT intCol, stringCol FROM test_catalog.test_schema.test_table  WHERE (intCol = ?) AND (stringCol = ?)";
    ...
    public void buildSql_WithEmptySchema_GeneratesQueryWithNull()
    {
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = HiveSqlUtils.buildSql(CATALOG_NAME, tableName, emptySchema, constraints, split, parameterValues);
    ...
    public void buildSql_WithCatalog_GeneratesQueryWithCatalog()
    {
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = HiveSqlUtils.buildSql(CATALOG_NAME, tableName, schema, constraints, split, parameterValues);
    ...
    public void buildSql_WithLimit_GeneratesQueryWithLimit()
    {
        Map<String, ValueSet> constraintMap = new LinkedHashMap<>();
        Schema schema = makeSchema(Collections.emptyMap());
        Constraints constraints = new Constraints(constraintMap, Collections.emptyList(), Collections.emptyList(), 10, Collections.emptyMap(), null);

        String expectedSql = "SELECT null FROM test_schema.test_table   LIMIT 10";
        
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = HiveSqlUtils.buildSql(null, tableName, schema, constraints, split, parameterValues);
    ...
    {
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = HiveSqlUtils.buildSql(HiveSqlUtilsTest.CATALOG_NAME, tableName, schema, constraints, split, parameterValues);
