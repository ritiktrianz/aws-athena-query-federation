...
public class SaphanaQueryBuilderTest
{
    ...
    private static final String PARTITION_COLUMN = "part_id";
    ...
    private SaphanaQueryFactory queryFactory;
    ...
    public void setUp()
    {
        queryFactory = new SaphanaQueryFactory();
    ...
    {
        String templateName = SaphanaQueryBuilder.getTemplateName();
    ...
    public void build_WithProjectionAndTableName_GeneratesCorrectSelectQuery()
    {
        ...
        assertFalse("Partition column should be excluded", sql.contains("\"part_id\""));
    }
    ...
    public void build_WithAllComponents_GeneratesCompleteQuery()
    {
        ...
        SaphanaQueryBuilder builder = queryFactory.createQueryBuilder();
        builder.withCatalog(null); // SAP HANA doesn't use catalog in FROM
        builder.withProjection(testSchema, split);
        builder.withTableName(TEST_TABLE);
        builder.withOrderByClause(constraints);
    ...
    public void withProjection_WithPartitionColumn_FiltersOutPartitionColumn()
    {
        ...
        assertFalse("Partition column should be filtered out", projection.contains("\"part_id\""));
        assertEquals("Projection should have 4 columns (excluding partition)", 4, projection.size());
    }

    @Test
    public void build_WithPartitionClause_GeneratesPartitionInFromClause()
    {
        Map<String, String> splitProperties = new HashMap<>();
        splitProperties.put(com.amazonaws.athena.connectors.saphana.SaphanaConstants.BLOCK_PARTITION_COLUMN_NAME, "1");
        
        Split splitWithPartition = mock(Split.class);
        when(splitWithPartition.getProperties()).thenReturn(splitProperties);
        when(splitWithPartition.getProperty(com.amazonaws.athena.connectors.saphana.SaphanaConstants.BLOCK_PARTITION_COLUMN_NAME)).thenReturn("1");
        ...
        assertNotNull("SQL should not be null", sql);
        assertTrue("SQL should contain PARTITION clause in FROM", sql.contains("PARTITION (1)"));
    }

    @Test
    public void build_WithAllPartitions_DoesNotIncludePartitionClause()
    {
        Map<String, String> splitProperties = new HashMap<>();
        splitProperties.put(com.amazonaws.athena.connectors.saphana.SaphanaConstants.BLOCK_PARTITION_COLUMN_NAME, 
                com.amazonaws.athena.connectors.saphana.SaphanaConstants.ALL_PARTITIONS);
        
        Split splitWithAllPartitions = mock(Split.class);
        when(splitWithAllPartitions.getProperties()).thenReturn(splitProperties);
        when(splitWithAllPartitions.getProperty(com.amazonaws.athena.connectors.saphana.SaphanaConstants.BLOCK_PARTITION_COLUMN_NAME))
                .thenReturn(com.amazonaws.athena.connectors.saphana.SaphanaConstants.ALL_PARTITIONS);
        ...
        assertNotNull("SQL should not be null", sql);
        assertFalse("SQL should not contain PARTITION clause", sql.contains("PARTITION"));
    }

    @Test(expected = NullPointerException.class)
    public void SaphanaQueryBuilder_WhenCreatedWithNullTemplate_ThrowsNullPointerException()
    {
        new SaphanaQueryBuilder(null);
    }
