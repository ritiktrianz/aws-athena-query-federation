...
public class ImpalaPredicateBuilderTest
{
    ...
    {
        ValueSet valueSet = createMultipleValueSet(INT_TYPE, 1, 2, 3);
        constraintMap.put("intCol", valueSet);
        fields.add(Field.nullable("intCol", INT_TYPE));
    ...
    {
        ValueSet valueSet = createRangeValueSet(INT_TYPE, 10, 20);
        constraintMap.put("intCol", valueSet);
        fields.add(Field.nullable("intCol", INT_TYPE));
    ...
    {
        ValueSet valueSet = createNullValueSet(VARCHAR_TYPE);
    ...
    public void buildConjuncts_WithNoConstraints_ReturnsEmptyList()
    {
        ...
        List<String> conjuncts = predicateBuilder.buildConjuncts(fields, createEmptyConstraints(), parameterValues, split);
    ...
    public void buildConjuncts_WithPartitionColumn_SkipsPartitionColumn()
    {
        ...
        assertEquals("Should have one conjunct", 1, conjuncts.size());
        assertTrue("Conjunct should contain intCol", conjuncts.get(0).contains("intCol"));
        assertTrue("Conjunct should not contain partitionCol", !conjuncts.get(0).contains("partitionCol"));
    }
    ...
    private ValueSet createMultipleValueSet(ArrowType type, Object... values)
    {
        SortedRangeSet.Builder builder = SortedRangeSet.newBuilder(type, false);
        for (Object value : values) {
            builder.add(new Range(Marker.exactly(allocator, type, value), Marker.exactly(allocator, type, value)));
        }
        return builder.build();
    }

    private ValueSet createRangeValueSet(ArrowType type, Object low, Object high)
    {
        return SortedRangeSet.newBuilder(type, false)
                .add(new Range(Marker.exactly(allocator, type, low), Marker.exactly(allocator, type, high)))
                .build();
    }

    private ValueSet createNullValueSet(ArrowType type)
    {
    ...
    {
        return new Constraints(Collections.emptyMap(), Collections.emptyList(), Collections.emptyList(), Constraints.DEFAULT_NO_LIMIT, Collections.emptyMap(), null);
    }
}
