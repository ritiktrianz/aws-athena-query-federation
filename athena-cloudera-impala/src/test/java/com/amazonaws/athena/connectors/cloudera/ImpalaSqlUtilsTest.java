...
public class ImpalaSqlUtilsTest
{
    ...
    public void buildSql_WithPartitionClause_GeneratesQueryWithPartition()
    {
        ...
        Split splitWithPartition = createSplitWithPartition();

        String expectedSql = "SELECT null FROM test_schema.test_table  WHERE p0";

        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = ImpalaSqlUtils.buildSql(tableName, schema, constraints, splitWithPartition, parameterValues);
    ...
    public void buildSql_WithEmptySchema_GeneratesQueryWithNull()
    {
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = ImpalaSqlUtils.buildSql(tableName, emptySchema, constraints, split, parameterValues);
    ...
    private Split createSplitWithPartition()
    {
        ...
        splitProperties.put(ImpalaConstants.BLOCK_PARTITION_COLUMN_NAME, "p0");
        Split splitWithPartition = mock(Split.class);
        when(splitWithPartition.getProperties()).thenReturn(splitProperties);
        when(splitWithPartition.getProperty(ImpalaConstants.BLOCK_PARTITION_COLUMN_NAME)).thenReturn("p0");
        return splitWithPartition;
    }
}
