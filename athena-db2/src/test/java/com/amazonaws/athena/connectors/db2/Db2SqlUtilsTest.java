...
public class Db2SqlUtilsTest
{
    ...
    @Test
    public void buildSql_WithPartitionClause_GeneratesQueryWithDatapartitionnum()
    {
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = getConstraints(constraintMap, Collections.emptyList());
        ...
        String sql = Db2SqlUtils.buildSql(tableName, schema, constraints, splitWithPartition, parameterValues);
    ...
    public void buildSql_WithEmptySchema_GeneratesQueryWithNull()
    {
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = Db2SqlUtils.buildSql(tableName, emptySchema, constraints, split, parameterValues);
    ...
    public void buildSql_WithPartitionAndConstraints_GeneratesCompleteQuery()
    {
        ...
        String expectedSql = "SELECT \"intCol\" FROM \"test_schema\".\"test_table\"  WHERE ((\"intCol\" > ? AND \"intCol\" < ?)) AND  DATAPARTITIONNUM(EMP_NO) = 2";
    ...
    {
        org.junit.Assert.assertNotNull("Query factory should not be null", Db2SqlUtils.getQueryFactory());
    }
